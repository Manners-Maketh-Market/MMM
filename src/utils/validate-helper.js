export const FormValidate = ({ email, pw, pwConfirm, nickName, phone, region, title }) => {
    // sign-in
    let disabled =
    ...
    if (pwConfirm) {
        // sign-up
        disabled = disabled || (pw !== pwConfirm && !/^[a-zA-Z0-9]$/.test(nickName) && !/^[0-9]+${,11}/.test(phone));
    }
    ...
    let errors = {
        ...
        nickName: !/^[a-zA-Z0-9ㄱ-ㅣ가-힣]+$/.test(nickName) && '닉네임에는 특수문자가 포함될 수 없습니다',
        phone: !/^[0-9]+${,11}/.test(phone) && '11자리의 숫자만 입력 가능합니다',
        region: !/^[ㄱ-ㅣ가-힣]+$/.test(region) && '한글만 입력 가능합니다, 한국 주소를 입력해주세요',
        title: !/^[a-zA-Z0-9ㄱ-ㅣ가-힣]+$/.test(title) && '제목은 특수문자가 포함될 수 없습니다',
    };
