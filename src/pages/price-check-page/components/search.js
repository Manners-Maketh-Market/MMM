...
const PriceSearch = () => {
    ...
    const { data: SearchProductList, refetch } = useQuery([PRODUCT_QUERY_KEY.SEARCH_PRODUCT_LIST], () =>
        Api.getSearchProduct(0, titles, 1)
    );

    // 판매완료된 상품 리스트
    const SearchSellProductList =
        SearchProductList && SearchProductList.product.filter((list) => list.status === '판매완료');
    ...
    const onArrowKeyPress = (e) => {
        if (e.key === 'ArrowUp') {
            ...
            navigate(`/MMM/pricecheckpage/${SearchSellProductList[selectedIndex].title}`);
